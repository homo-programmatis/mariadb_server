...
static void innobase_kill_query(handlerton*, THD* thd, enum thd_kill_levels)
{
  ...
  /* Here we need protect thd from concurrent kill or delete */
  wsrep_thd_lock_check(thd);
  if (trx_t* trx= thd_to_trx(thd))
  {
    if (trx_id_t trx_id= trx->id)
    {
      ut_ad(trx->mysql_thd == thd);
      /* We need to release THD::LOCK_thd_data to follow mutexing
      order rules here. Order should be:
      lock_sys => trx_mutex => THD::LOCK_thd_data */
	...
	wsrep_thd_UNLOCK(victim_trx->mysql_thd);
      }
      lock_mutex_exit();
    }
  }
